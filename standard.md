...
```
\box(sizeR,align){Something}
```
在大括号中可继续嵌套其他内容，这对应html中的 "div"
...
```
\box(sizeR=100%,align=center,background-color=transparent){...}
```
...
2> 对内容进行 html 转义。  
3> 对 `\box` 进行括号匹配。将其转换为 div。  
4> 从前到后遍历文章内的 `\`，使用正则表达式匹配 `\\[a-zA-Z-\\]+` 并校验参数是否合法。只要发现不合法，立刻抛出错误并停止解析，若无错误加入 html 标签。  
5> 将公式和代码加回去。
